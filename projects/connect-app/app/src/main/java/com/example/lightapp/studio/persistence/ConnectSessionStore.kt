...
/**
 * connect(7) persistence — wraps the shell's game-agnostic [SessionStore] with the
 * connect-specific encode/decode.
 *
 * The shell never learns what a connect session looks like: it stores opaque string
 * fields per game and hands them back. This class owns the field names and the board
 * encoding, and exposes connect-typed accessors to the game's ViewModel and
 * MainActivity.
 */
class ConnectSessionStore(context: Context) {
    ...
    // ── game sessions ──────────────────────────────────────────────────────────
    private val gameId = "connect"

    private fun decode(fields: Map<String, String>): ConnectSession? {
        val board = ConnectCodec.decodeBoard(fields["board"]) ?: return null
    ...
    private fun encode(session: ConnectSession): Map<String, String> = mapOf(
        "board" to ConnectCodec.encodeBoard(session.board),
    ...
    /** The connect session, or null when none is stored. */
    fun session(): Flow<ConnectSession?> = store.sessionFields(gameId).map { fields ->
        fields?.let { decode(it) }
    }

    suspend fun saveSession(session: ConnectSession) {
        store.storeSession(gameId, encode(session))
    }

    suspend fun clearSession() = store.clearSession(gameId)

    /** All resume-card snapshots — keyed by game id. For connect, just this one. */
     ...
     */
    fun daily(day: Long): Flow<ConnectSession?> = store.dailyFields(gameId, day).map { fields ->
        fields?.let { decode(it) }
    }

    suspend fun saveDaily(day: Long, session: ConnectSession) {
