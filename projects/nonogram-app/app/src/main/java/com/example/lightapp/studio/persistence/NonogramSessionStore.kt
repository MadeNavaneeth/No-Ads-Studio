...
/**
 * nonogram(10) persistence — wraps the shell's game-agnostic [SessionStore] with the
 * nonogram-specific encode/decode.
 *
 * The shell never learns what a nonogram session looks like: it stores opaque string
 * fields per game and hands them back. This class owns the field names and the
 * bitmap/cells encoding, and exposes nonogram-typed accessors to the game's
 * ViewModel and MainActivity.
 */
class NonogramSessionStore(context: Context) {
    ...
    // ── game sessions ──────────────────────────────────────────────────────────
    private val gameId = "nonogram"

    private fun decode(fields: Map<String, String>): NonogramSession? {
        val solution = NonogramCodec.decodeBitmap(fields["solution"]) ?: return null
        val cells = NonogramCodec.decodeCells(fields["cells"]) ?: return null
    ...
    private fun encode(session: NonogramSession): Map<String, String> = mapOf(
        "solution" to NonogramCodec.encodeBitmap(session.solution),
        "cells" to NonogramCodec.encodeCells(session.cells),
    ...
    /** The nonogram session, or null when none is stored. */
    fun session(): Flow<NonogramSession?> = store.sessionFields(gameId).map { fields ->
        fields?.let { decode(it) }
    }

    suspend fun saveSession(session: NonogramSession) {
        store.storeSession(gameId, encode(session))
    }

    suspend fun clearSession() = store.clearSession(gameId)

    /** All resume-card snapshots — keyed by game id. For nonogram, just this one. */
     ...
     */
    fun daily(day: Long): Flow<NonogramSession?> = store.dailyFields(gameId, day).map { fields ->
        fields?.let { decode(it) }
    }

    suspend fun saveDaily(day: Long, session: NonogramSession) {
