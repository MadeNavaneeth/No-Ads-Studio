...
class MinesweeperViewModel(
    ...
    private suspend fun restoreOrGenerate() {
        val saved = store.minesweeperSession().first()
        ...
        when {
            ...
            state.isComplete || state.detonated -> {
                viewModelScope.launch { store.clearMinesweeperSession() }
    ...
    private fun startDaily() {
        ...
        viewModelScope.launch {
            val saved = store.dailyMinesweeper(day).first()
            if (saved != null) {
    ...
    private suspend fun clearSlot(state: MinesweeperState) {
        if (state.dailyDay != null) store.clearDailyMinesweeper() else store.clearMinesweeperSession()
    }
    ...
    private fun persist(state: MinesweeperState) {
        viewModelScope.launch {
            ...
            if (day != null) store.saveDailyMinesweeper(day, state.toSession())
            else store.saveMinesweeperSession(state.toSession())
        }
    }
