...
/**
 * The catalog for the standalone **minesweeper** flavour — one game.
 *
 ...
 *
 * Discoverability. People search stores for "minesweeper", not for "puzzle studio", so
 * a standalone listing can rank for the term that has the traffic and a bundled one
 * cannot. The install is also smaller, and one game's reviews cannot drag the others
 * down. Like `gameNonogram`, `gameMinesweeper` carries no vendored GPL code, so this
 * listing ships without an obligation it never earned.
 *
