...
/**
 * The catalog for the standalone **wordsearch** flavour — one game.
 *
 ...
 *
 * Discoverability. People search stores for "word search", not for "puzzle studio",
 * so a standalone listing can rank for the terms that have the traffic and a bundled
 * one cannot. The install is also smaller, and one game's reviews cannot drag the
 * others down. `gameWordsearch` carries no vendored code at all — the generator and
 * the bundled word pool are our own — so this listing has a clean licence story.
 *
