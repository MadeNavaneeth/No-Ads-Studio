...
class WordsearchViewModel(
    ...
    private suspend fun restoreOrGenerate() {
        val saved = store.wordsearchSession().first()
        ...
        when {
            ...
            state.isComplete -> {
                viewModelScope.launch { store.clearWordsearchSession() }
    ...
    private fun startDaily() {
        ...
        _ui.value = WordsearchUi.Generating
        generateJob = viewModelScope.launch {
            val saved = store.dailyWordsearch(day).first()
            if (saved != null) {
    ...
    private suspend fun clearSlot(state: WordsearchState) {
        if (state.dailyDay != null) store.clearDailyWordsearch() else store.clearWordsearchSession()
    }
    ...
    private fun persist(state: WordsearchState) {
        viewModelScope.launch {
            ...
            if (day != null) store.saveDailyWordsearch(day, state.toSession())
            else store.saveWordsearchSession(state.toSession())
        }
    }
