...
class BlockViewModel(
    ...
    private suspend fun restoreOrDeal() {
        val saved = store.blockSession().first()
    ...
    private fun startDaily() {
        ...
        viewModelScope.launch {
            val saved = store.dailyBlock(day).first()
            if (saved != null) {
    ...
    private suspend fun clearSlot(state: BlockState) {
        if (state.dailyDay != null) store.clearDailyBlock() else store.clearBlockSession()
    }
    ...
    private fun persist(state: BlockState) {
        viewModelScope.launch {
            ...
            if (day != null) store.saveDailyBlock(day, state.toSession())
            else store.saveBlockSession(state.toSession())
        }
    }
