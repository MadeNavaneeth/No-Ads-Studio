 ...
 *
 * Games are resolved through [GameRegistry] and rendered via the `GameDefinition`
 * factory, so this file knows nothing about Sudoku specifically.
 *
 * ## Structure
 *
...
fun NothingGamesApp(
    ...
    val standalone = GameRegistry.standalone
                ...
                AnimatedContent(
                    ...
                    GuardedScreen(name = current.guardName(), strict = BuildConfig.DEBUG) {
                        when (current) {
                            ...
                            is Screen.Game -> {
                                android.util.Log.w("NothingGames", "Screen.Game getById(${current.gameId}) from ${GameRegistry.games.map { it.id }}")
                                val game = GameRegistry.getById(current.gameId)
                                if (game == null) {
                                    // A registry id that no longer resolves. Fail visibly
                            ...
                            is Screen.ChooseDifficulty -> {
                                android.util.Log.w("NothingGames", "ChooseDifficulty getById(${current.gameId}) from ${GameRegistry.games.map { it.id }}")
                                val game = GameRegistry.getById(current.gameId)
                                if (game == null) {
                                    NothingBackground {
                                        Label(
