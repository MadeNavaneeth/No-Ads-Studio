 ...
 *
 * Placements travel as `WORD:start:end` triples over the same `session.<id>.*` key
 * shape the other games use. Direction and length re-derive from the endpoints, and
 * the restore step (the game's own, which owns its invariants) checks the triple
 * against the letter grid before trusting it.
 *
