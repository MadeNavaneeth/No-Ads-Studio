...
class AkariViewModel(
    ...
    private suspend fun restoreOrGenerate() {
        val saved = store.akariSession().first()
        ...
        when {
            ...
            state.isComplete -> {
                viewModelScope.launch { store.clearAkariSession() }
    ...
    private fun startDaily() {
        ...
        _ui.value = AkariUi.Generating
        generateJob = viewModelScope.launch {
            val saved = store.dailyAkari(day).first()
            if (saved != null) {
    ...
    private suspend fun clearSlot(state: AkariState) {
        if (state.dailyDay != null) store.clearDailyAkari() else store.clearAkariSession()
    }
    ...
    private fun persist(state: AkariState) {
        viewModelScope.launch {
            ...
            if (day != null) store.saveDailyAkari(day, state.toSession())
            else store.saveAkariSession(state.toSession())
        }
    }
