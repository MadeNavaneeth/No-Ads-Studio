...
class QQWing(type: GameType, difficulty: GameDifficulty) {
    ...
    /**
     * This board's own random source.
     *
     * Upstream kept one `Random` in the shared static state, so every QQWing instance
     * in the process drew from the same stream. That is what makes a generated puzzle
     * different each run — and it is exactly why a seeded run was impossible: with
     * several instances (the controller's worker threads) interleaving draws from one
     * shared stream, a fixed seed could not pin down the result.
     *
     * Per-instance, seeded through [setRandom], a fixed seed yields the same puzzle
     * every run. That is the whole determinism contract the local daily puzzle rests on.
     */
    ...
    /**
     * Seeds this board's own random source.
     *
     * Same seed, same puzzle: every random draw during [generatePuzzleSymmetry] and
     * [solve] comes from this instance's stream, so two boards seeded identically and
     * driven identically produce identical output regardless of what any other QQWing
     * instance is doing. The daily puzzle seeds with the UTC day number.
     */
    ...
    /**
     * Fisher–Yates over this board's two random-order arrays, drawing from this
     * board's own stream. Instance method, not a companion static: a shuffle is only
     * deterministic if the stream it draws from is the board's own.
     */
    ...
    /**
     * A random symmetry from this board's own stream — never NONE or RANDOM.
     */
