...
import com.example.lightapp.studio.persistence.BinairoSessionStore
import com.example.lightapp.studio.persistence.GameStats
import kotlinx.coroutines.launch

/**
 * Entry point for the standalone binairo(10) app.
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            val store = remember { BinairoSessionStore(applicationContext) }
            ...
            val resume = allSummaries["binairo"]?.let { snapshot ->
                ResumeState(
